<?
    ...
    switch($topic) {
        case "payment":
            ...
            $orden_compra = MercadoPago\MerchantOrder::find_by_id($pago->order->id);
            break;
        case "orden_compra":
            $orden_compra = MercadoPago\MerchantOrder::find_by_id($id);
            break;
    }

    $monto_pagado = 0;
    foreach ($orden_compra->payments as $pago) {
	...
	$mensaje = "";
    if ($monto_pagado >= $orden_compra->total_amount) {	
		$mensaje = "Pago total";
        if (count($orden_compra->shipments)>0) {
			foreach ($orden_compra->shipments as $shipment) {
				if ($shipment->status == "ready_to_ship") {
					$mensaje = "Pago total. Imprime la etiqueta para enviar el producto.";
				}
			}			
        }
    } else {
        $mensaje = "No pagado todavía";
    }
?>
